...
- Adopt a single canonical breakpoint set — **480 / 768 / 1024** — recorded as a
  ...
  literals.
- Introduce `--field-size` for every form control: 14px by default, 16px under
  `@media (pointer: coarse)`, switched once in `tokens.css` so no call site carries its
  own query. Keyed on the pointer rather than a width, because focus-zoom follows touch
  and not window size. Body text stays 14px.
...
- `design-system`: *Typography conventions* — form controls (`input`, `select`,
  `textarea`) move from 14px to 16px on touch devices while body text stays 14px, with
