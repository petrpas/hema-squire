...
- [ ] 8.3 (BLOCKED — the browser window in this session would not resize past 500px, so the 768/1024 bands were verified from the compiled stylesheet and computed styles rather than by eye) DevTools pass at 360 / 390 / 768 / 1024, each width walking the whole path: sign-in → account creation → home → tournament detail → registration → payment slip → profile → sign-out.
