# payments-clearing Specification

## Purpose
Define how a tournament's imported payments are removed: what a clear destroys,
what it leaves standing, and when it is refused. Clearing exists so a misread
statement is a recoverable mistake rather than a permanent one — the organizer
takes the money back out, corrects the file, and imports again against a clean
slate. It is refused wherever money has already been credited, because a payment
the tournament has acted on is history, not draft data.
