...
## Purpose
Define the Bureau 1952 visual design system for Squire UI: a single CSS token source, typography conventions, ledger table behavior, form conventions, button hierarchy, tags/stamps, modal and payment-slip framing, icon usage, an accessibility baseline, a bounded personality "wink budget", and a global list of prohibited patterns — so all screens read as one coherent, restrained, paper-ledger-inspired system.
