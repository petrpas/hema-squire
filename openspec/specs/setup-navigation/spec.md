...
Govern how the Setup phase's settings pane is navigated and committed: the six
tabs and which sections belong to each, the default and persistence of the
selection, the per-tab incompleteness markers now that the checklist lives on
`PUBLISH`, the preservation of unsaved edits across tab switches, the
...
### Requirement: Setup settings are navigated by six tabs
...
- **WHEN** the organizer moves through all six tabs
