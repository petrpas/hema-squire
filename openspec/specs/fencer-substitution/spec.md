# fencer-substitution Specification

## Purpose
Define what it means for a seat on a tournament's fencer list to change
hands: one organizer action replaces the person behind a row while the row
itself — its number, its place in the order and in every queue, its variable
symbol, its total, its items and everything credited against it — stands
unchanged. The substitute is named through the HEMA Ratings index, the seat's
contact address decides which account holds it, both parties are told once
each, and the whole decision persists as a single withdrawable rule.
