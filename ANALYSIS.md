...
## 4. Inferred defaults — reviewed and settled by the owner (2026-07-17)

1. **Substitute queue** — CONFIRMED: capacity + waitlist with *manual* admission by the organizer when a spot frees.
2. **Rule ordering** — CONFIRMED: creation order, latest-wins per field.
3. **Merge precedence** (same-hr_id dedup) — AMENDED: no automatic merge; same-hr_id groups queue for organizer review with an LLM-prepared proposal prefilled with the most recent explicit value per field. Confirmation persists as a rule.
4. **Meta-journal** — AMENDED: the spec now mandates an append-only journal of rule creation/deletion events (actor, timestamp), excluded from replay. Data-side "deleted rule = as if it never existed" unchanged.
5. **Repeat-export surgical semantics** — CONFIRMED: sheet target only; canonical JSON is always a full regeneration.
6. **Unpaid on public list** — SETTLED: per-tournament setting with default greyed "unconfirmed".
