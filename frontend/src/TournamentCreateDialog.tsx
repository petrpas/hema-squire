...
/** Whether this account may create a tournament: the Organizer role or above,
 *  or the deployment Owner (spec `tournament-admin`, In-app tournament
 *  creation). Lives with the dialog so the two places offering it — the
 *  account menu and the picker — cannot disagree about who is offered it. */
export function canCreateTournament(account: Account | null): boolean {
  return account !== null && (account.role !== "fencer" || account.is_deployment_owner);
}
