...
Imported records sharing an hr_id SHALL be merged automatically into a single record: inputs ordered by registration time, merged by an LLM, with a merge note recorded. The default precedence is the most recent explicit value per field; superseded values remain visible in the audit trail.

#### Scenario: Fencer registered twice
- **WHEN** two imported rows carry the same hr_id
- **THEN** one merged record remains, carrying a note describing the merge
