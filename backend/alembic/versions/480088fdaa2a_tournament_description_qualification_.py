...
down_revision: str | Sequence[str] | None = "b4d9c1e07a52"
...
def upgrade() -> None:
    ...
    with op.batch_alter_table("tournaments", schema=None) as batch_op:
        ...
        batch_op.add_column(
            sa.Column("qualification_open", sa.Boolean(), nullable=False, server_default=sa.true())
        )
        batch_op.add_column(sa.Column("qualification_criteria", sa.Text(), nullable=True))
        batch_op.add_column(sa.Column("organizers", sa.JSON(), nullable=False, server_default="[]"))
...
def downgrade() -> None:
    ...
    for row in rows:
        ...
        conn.execute(
            tournaments.update().where(tournaments.c.id == row.id).values(organizer_names=names)
        )
