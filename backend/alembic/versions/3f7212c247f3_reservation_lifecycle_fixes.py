...
down_revision: str | Sequence[str] | None = "90aeb7ba0f10"
...
def upgrade() -> None:
    with op.batch_alter_table("tournaments", schema=None) as batch_op:
        batch_op.add_column(
            sa.Column("expiry_grace_hours", sa.Integer(), nullable=False, server_default="48")
        )
    ...
    op.execute(
        "UPDATE registrations SET amount_paid_cents = total_amount * 100 WHERE state = 'paid'"
    )
