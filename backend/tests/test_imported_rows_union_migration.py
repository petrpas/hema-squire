...
def _seed(db_path: Path) -> None:
    ...
    for batch_id, keys in batches.items():
        ...
        for row_number, key in enumerate(keys, start=1):
            _insert(conn, "imported_rows", batch_id=batch_id, tournament_id=1,
                    row_number=row_number, key=key, raw="{}")
    for number, key in enumerate(batches[3], start=1):
        _insert(conn, "sheet_row_numbers", tournament_id=1, row_id=f"imp:{key}",
                number=number)
    conn.commit()
    conn.close()
...
def migrated_db(tmp_path) -> Path:
    db_path = tmp_path / "imported_rows_union.sqlite"
...
def test_one_row_survives_per_key_and_it_is_the_earliest(migrated_db):
    ...
    rows = conn.execute(
        "SELECT key, batch_id FROM imported_rows ORDER BY key"
    ).fetchall()
    conn.close()
...
def test_the_numbers_are_untouched(migrated_db):
    conn = sqlite3.connect(migrated_db)
    numbers = dict(
        conn.execute("SELECT row_id, number FROM sheet_row_numbers ORDER BY number")
    )
    conn.close()
...
def test_a_key_cannot_be_imported_twice_into_one_tournament(migrated_db):
    ...
    with pytest.raises(sqlite3.IntegrityError):
        _insert(conn, "imported_rows", batch_id=3, tournament_id=1,
                row_number=9, key="aaaa", raw="{}")
    conn.close()


def test_the_same_key_in_another_tournament_is_fine(migrated_db):
    conn = sqlite3.connect(migrated_db)
    _insert(conn, "tournaments", id=2, slug="other", display_name="Other",
            date="2026-12-06", language="cs", organizers="[]", discounts="[]",
            hr_category_map="{}", local_currency="CZK", vs_year=27, vs_series=1)
    _insert(conn, "import_batches", id=4, tournament_id=2, filename="regs.csv",
            uploaded_by=1, row_count=1)
    _insert(conn, "imported_rows", batch_id=4, tournament_id=2, row_number=1,
            key="aaaa", raw="{}")
    conn.commit()
    conn.close()
...
def test_downgrade_restores_per_batch_uniqueness(migrated_db):
    ...
    _insert(conn, "imported_rows", batch_id=3, tournament_id=1, row_number=9,
            key="aaaa", raw="{}")
    conn.commit()
    conn.close()
